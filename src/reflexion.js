 ...
 *
 * In-memory with optional JSON file persistence via REFLEXION_DATA_DIR.
 */
...
/** @type {Map<string, object>} decision_id -> decision record */
const decisions = new Map();

/** @type {Map<string, object[]>} agent_id -> metric entries */
const metrics = new Map();

/** @type {Map<string, object[]>} agent_id -> goals */
const goals = new Map();

const DATA_DIR = process.env.REFLEXION_DATA_DIR || join(homedir(), '.agent-reflexion');
...
function loadFromDisk() {
  ...
  try {
    if (existsSync(DECISIONS_FILE)) {
      const raw = JSON.parse(readFileSync(DECISIONS_FILE, 'utf-8'));
      for (const [k, v] of Object.entries(raw)) decisions.set(k, v);
    }
  } catch { /* ignore corrupt files */ }
  try {
    if (existsSync(METRICS_FILE)) {
      const raw = JSON.parse(readFileSync(METRICS_FILE, 'utf-8'));
      for (const [k, v] of Object.entries(raw)) metrics.set(k, v);
    }
  } catch { /* ignore */ }
  try {
    if (existsSync(GOALS_FILE)) {
      const raw = JSON.parse(readFileSync(GOALS_FILE, 'utf-8'));
      for (const [k, v] of Object.entries(raw)) goals.set(k, v);
    }
  } catch { /* ignore */ }
}
...
function saveToDisk() {
  ...
  writeFileSync(DECISIONS_FILE, JSON.stringify(decisionsObj, null, 2));
  writeFileSync(METRICS_FILE, JSON.stringify(metricsObj, null, 2));
  writeFileSync(GOALS_FILE, JSON.stringify(goalsObj, null, 2));
}
...
function getAgentDecisions(agent_id, task_type, time_range_days) {
  const cutoff = Date.now() - time_range_days * 86400000;
  const results = [];
  for (const d of decisions.values()) {
    if (d.agent_id !== agent_id) continue;
    if (task_type && d.task_type !== task_type) continue;
...
export function logDecision(agent_id, task_type, decision, reasoning, expected_outcome, confidence, tags = []) {
  ...
  decisions.set(decision_id, record);
  saveToDisk();
...
export function logOutcome(decision_id, actual_outcome, status, quality_score = null, lessons_learned = null) {
  const record = decisions.get(decision_id);
  if (!record) {
...
export function analyzePatterns(agent_id, task_type = null, time_range_days = 30, min_samples = 5) {
  ...
  // Confidence calibration — average confidence vs actual success rate
  ...
  // Trend detection — split into halves and compare
  ...
  // By-task-type breakdown for improving/declining
  ...
  return {
    ...
    trend: {
      ...
      direction: secondSuccessRate > firstSuccessRate + 0.05 ? 'improving' : secondSuccessRate < firstSuccessRate - 0.05 ? 'declining' : 'stable',
    },
    improving_areas,
    declining_areas,
  };
...
export function getSuggestions(agent_id, task_type, current_context = null) {
  ...
  // Gather all resolved decisions for this agent
  const allResolved = [];
  for (const d of decisions.values()) {
    if (d.agent_id === agent_id && d.resolved) allResolved.push(d);
  }
  ...
  // 1. Confidence calibration suggestion
  ...
  if (taskFailures.length >= 2) {
    // Look for common words in failure outcomes
    const outcomeWords = {};
  ...
  // 3. Lessons learned from past successes
  ...
  // 4. Cross-task-type insights — what works elsewhere?
  ...
  // 6. Recency suggestion — are you repeating recent failures?
  ...
  // If no suggestions generated, give a positive one
...
export function trackMetric(agent_id, metric_name, value, context = null) {
  ...
  if (!metrics.has(agent_id)) metrics.set(agent_id, []);
  const agentMetrics = metrics.get(agent_id);

  const entry = { metric_id, metric_name, value, context, timestamp };
  agentMetrics.push(entry);

  // Compute trend and rolling average for this metric
  const metricValues = agentMetrics
...
export function getPerformanceReport(agent_id, time_range_days = 7) {
  ...
  // Improvement trend — compare this period vs previous period
  const prevDecisions = getAgentDecisions(agent_id, null, time_range_days * 2)
    .filter(d => d.resolved)
    .filter(d => new Date(d.timestamp).getTime() < Date.now() - time_range_days * 86400000);
  ...
  // Metrics summary
  const agentMetrics = metrics.get(agent_id) || [];
  const metricNames = [...new Set(agentMetrics.map(m => m.metric_name))];
  const metrics_summary = metricNames.map(name => {
    const vals = agentMetrics.filter(m => m.metric_name === name).map(m => m.value);
    return {
      metric_name: name,
  ...
  // Goals progress
  const agentGoals = goals.get(agent_id) || [];
  const goals_progress = agentGoals
    .filter(g => g.status === 'active')
    .map(g => {
      const metricVals = agentMetrics
        .filter(m => m.metric_name === g.metric_name)
        .map(m => m.value);
...
export function setGoal(agent_id, goal_name, metric_name, target_value, deadline_days) {
  ...
  // Find baseline — latest value for this metric, or 0
  const agentMetrics = metrics.get(agent_id) || [];
  const metricVals = agentMetrics.filter(m => m.metric_name === metric_name).map(m => m.value);
  const baseline_value = metricVals.length > 0 ? metricVals[metricVals.length - 1] : 0;
  ...
  if (!goals.has(agent_id)) goals.set(agent_id, []);
  goals.get(agent_id).push(goal);
  saveToDisk();
...
export function checkGoals(agent_id) {
  ...
  const result = agentGoals
    ...
    .map(g => {
      ...
      // On-track calculation: expected progress given elapsed time
      const expected_progress = g.deadline_days > 0 ? (days_elapsed / g.deadline_days) * 100 : 100;
      const on_track = progress_percent >= expected_progress * 0.8; // 80% of expected pace
